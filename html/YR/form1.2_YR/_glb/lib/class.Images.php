...
//		echo $image_save_func;
		...
		if($image_save_func == "imageJPG" || $image_save_func == "ImageJPEG"){
		    ...
		    $pngQuality = round($jpgQuality/10)-1;

			if(!$image_save_func($imageC, $this->tmpfile, $pngQuality)){
				throw new Exception("Cannot save file ".$this->tmpfile);
			}//if
	...
	protected function getFunctionNames(){
		...
		switch ($this->getType()){
			case 'jpeg':
	            $Chk_JPEG = @ImageCreateFromJPEG($this->image);
	            $Chk_GIF = @ImageCreateFromGIF($this->image);
	            $Chk_PNG = @ImageCreateFromPNG($this->image);
	            ...
	            else if( $Chk_GIF !== false )
	            {
				    $image_create_func = 'ImageCreateFromGIF';
				    $image_save_func = 'ImageGIF';
            		break;
	            }
	            else if( $Chk_PNG !== false )
	            {
