...
<? 
		...
		/////////////////////////////////////////////////////////////////////////////
		// 사용자 메뉴 및 페이지 생성 2023.05.03 HJ, 20.05.06 islro
		/////////////////////////////////////////////////////////////////////////////
		$page_block_arr = array("service");		// 페이지 폴더 블럭 설정
		for($b=0; $b < count($page_block_arr); $b++) {
			$page_key = $page_block_arr[$b] ;
			if(preg_match("/".$page_key."\/(auditor_)/i", $_SERVER["PHP_SELF"])) { // 페이지 경로로 페이지 블럭 설정
				// 페이지 블럭별 왼쪽 메뉴 세팅
				$pages = array();
				if($page_key == "service"){
					$page_folder = "/gpc_eng/theme/gpcert/service/" ; // 사용자 디렉토리 설정
					$pages[0][$page_key] = array("SERVICES"); // 왼쪽 메뉴 타이틀
					$pages[1][$page_key] = array("Auditor Certification", $_SERVER["PHP_SELF"], "_self"); // 왼쪽 메뉴 1
					$pages[2][$page_key] = array("Training Provider Designation", $page_folder."training_introduction.php", "_self"); // 왼쪽 메뉴 2
					$pages[3][$page_key] = array("Examination", $page_folder."exam_introduction.php", "_self"); // 왼쪽 메뉴 3
					$pages[4][$page_key] = array("Training", $page_folder."edu_introduction.php", "_self"); // 왼쪽 메뉴 4
				}
			...
			if(preg_match("/".$page_key."\/(training_)/i", $_SERVER["PHP_SELF"])) { // 페이지 경로로 페이지 블럭 설정
				...
				if($page_key == "service"){
					$page_folder = "/gpc_eng/theme/gpcert/service/" ; // 사용자 디렉토리 설정
					$pages[0][$page_key] = array("SERVICES"); // 왼쪽 메뉴 타이틀
					$pages[1][$page_key] = array("Auditor Certification", $page_folder."auditor_scheme.php", "_self"); // 왼쪽 메뉴 1
					$pages[2][$page_key] = array("Training Provider Designation", $_SERVER["PHP_SELF"], "_self"); // 왼쪽 메뉴 2
					$pages[3][$page_key] = array("Examination", $page_folder."exam_introduction.php", "_self"); // 왼쪽 메뉴 3
					$pages[4][$page_key] = array("Training", $page_folder."edu_introduction.php", "_self"); // 왼쪽 메뉴 4
				}
				for($i=0; $i<count($pages); $i++) { 				
			...
			if(preg_match("/".$page_key."\/(exam_)/i", $_SERVER["PHP_SELF"])) { // 페이지 경로로 페이지 블럭 설정
				...
				if($page_key == "service"){
					...
					$pages[1][$page_key] = array("Auditor Certification", $page_folder."auditor_scheme.php", "_self"); // 왼쪽 메뉴 1
					$pages[2][$page_key] = array("Training Provider Designation", $page_folder."training_introduction.php", "_self"); // 왼쪽 메뉴 2
					$pages[3][$page_key] = array("Examination", $_SERVER["PHP_SELF"], "_self"); // 왼쪽 메뉴 3
					$pages[4][$page_key] = array("Training", $page_folder."edu_introduction.php", "_self"); // 왼쪽 메뉴 4
				}
				for($i=0; $i<count($pages); $i++) { 				
					$menu_name = $pages[$i][$page_key][0] ;
			...
			if(preg_match("/".$page_key."\/(edu_)/i", $_SERVER["PHP_SELF"])) { // 페이지 경로로 페이지 블럭 설정
				// 페이지 블럭별 왼쪽 메뉴 세팅
				$pages = array();
				if($page_key == "service"){
					$page_folder = "/gpc_eng/theme/gpcert/service/" ; // 사용자 디렉토리 설정
					...
					$pages[2][$page_key] = array("Training Provider Designation", $page_folder."training_introduction.php", "_self"); // 왼쪽 메뉴 2
					$pages[3][$page_key] = array("Examination", $page_folder."exam_introduction.php", "_self"); // 왼쪽 메뉴 3
					$pages[4][$page_key] = array("Training", $_SERVER["PHP_SELF"], "_self"); // 왼쪽 메뉴 4
				}
				for($i=0; $i<count($pages); $i++) { 				
					$menu_name = $pages[$i][$page_key][0] ;
					$menu_link = $pages[$i][$page_key][1] ;
...
<div class="left_cs">
	...
	<p class="info">Closed on weekends and holidays</p>
	<dl>
