...
// 메뉴테이블 생성
if( !isset($g5['menu_en_table']) ){
    die('<meta charset="utf-8">dbconfig.php 파일에 <strong>$g5[\'menu_table\'] = G5_TABLE_PREFIX.\'menu\';</strong> 를 추가해 주세요.');
}

if(!sql_query(" DESCRIBE {$g5['menu_en_table']} ", false)) {
    sql_query(" CREATE TABLE IF NOT EXISTS `{$g5['menu_en_table']}` (
...
$sql = " select * from {$g5['menu_en_table']} order by me_id ";
